...
def get_dir_probs(classifier, dir_path):
    ...
    return child_prob, embeddings, img_features
...
def measure_xclip_inference_time(classifier, X_test, img_features_test):
    """Measure XClip inference time"""
    # Run multiple times to get stable timing
    num_runs = 3
    timing_results = []
    ...
    for _ in range(num_runs):
        start_time = time.time()
        _ = classifier.classify_child_presence(X_test, img_features_test)
        end_time = time.time()
        timing_results.append(end_time - start_time)
...
def main():
    ...
    # Initialize the XClip classifier
    print("Initializing XClip classifier...")
    ...
    child_probs, child_embeddings, child_img_features = get_dir_probs(classifier, child_dir)
    
    print("\n--- Processing Non-Child Videos ---")
    non_child_probs, non_child_embeddings, non_child_img_features = get_dir_probs(classifier, non_child_dir)
    
    # Evaluate original XClip model
    print("\n--- Evaluating Original XClip Model ---")
    ...
    best_threshold_idx = np.argmax(f1s)
    best_threshold = thresholds[best_threshold_idx]
    best_f1 = f1s[best_threshold_idx]
    
    print(f"XClip Best threshold: {best_threshold:.2f} with F1 score: {best_f1:.4f}")
    
    # Measure XClip inference speed
    print("\n--- Measuring XClip Inference Speed ---")
    # Use a sample of the data to measure speed
    sample_size = min(1000, len(child_embeddings))
    sample_embeddings = child_embeddings[:sample_size]
    sample_img_features = child_img_features[:sample_size]
    
    xclip_timing = measure_xclip_inference_time(classifier, sample_embeddings, sample_img_features)
    print(f"XClip inference speed: {xclip_timing['samples_per_second']:.1f} samples/second")
    print(f"XClip inference time: {xclip_timing['inference_time']:.4f} seconds for {sample_size} samples")
    
    # Plot XClip results
    fig, ax = plt.subplots()
    ax.plot(thresholds, f1s)
    ax.axvline(x=best_threshold, color='r', linestyle='--', 
              label=f'Best threshold: {best_threshold:.2f}')
    ax.set_xlabel('Threshold')
    ax.set_ylabel('F1 Score')
    ax.set_title('XClip F1 Score vs Threshold')
    ax.legend()
    plt.tight_layout()
    fig.savefig('./figs/xclip_thres_f1.png')
    ...
    model_timings = {
        "XClip": {
            "f1": best_f1,
            "samples_per_second": xclip_timing['samples_per_second']
        }
    }
    ...
    print(f"XClip Best F1: {best_f1:.4f}")
    
    # Create comparison figure for all models
    fig, ax = plt.subplots(figsize=(12, 8))
    ax.plot(thresholds, f1s, label='XClip', linewidth=2)
    
    for model_name, results in model_results.items():
        print(f"{model_name} Best F1: {results['best_f1']:.4f}")
        improvement = (results['best_f1'] - best_f1) / best_f1 * 100
        print(f"{model_name} Improvement: {improvement:.2f}%")
    ...
    fig, ax = plt.subplots(figsize=(12, 8))
    model_names = ['XClip'] + list(model_results.keys())
    f1_scores = [best_f1] + [results['best_f1'] for results in model_results.values()]
    ...
    # Add XClip
    xclip_acc = accuracy_score(true_labels, combined_probs > best_threshold)
    xclip_prec = precision_score(true_labels, combined_probs > best_threshold)
    xclip_rec = recall_score(true_labels, combined_probs > best_threshold)
    print(f"{'XClip':<25} {xclip_acc:.4f}{'':<5} {xclip_prec:.4f}{'':<5} {xclip_rec:.4f}{'':<5} {best_f1:.4f}{'':<5} {best_f1:.4f}{'':<5} {xclip_timing['samples_per_second']:.1f}")
    
    # Add other models - use f1_sorted_names to maintain consistency
    for name in f1_sorted_names:  
        if name == 'XClip':  # Skip XClip which we just added
            continue
    ...
    if best_model_name == 'XClip':
        print("\nXClip performed best for F1 score among all models!")
    else:
        best_improvement = (model_timings[best_model_name]['f1'] - best_f1) / best_f1 * 100
        print(f"\nBest model for F1 score: {best_model_name} with {best_improvement:.2f}% improvement over XClip")
    ...
    speedup = fastest_speed / xclip_timing['samples_per_second']
    print(f"Fastest model: {fastest_model} with {fastest_speed:.1f} samples/second ({speedup:.1f}x faster than XClip)")
